...
class DecisionTreeClassifier:
    ...
    def _build_node(self, node: DecissionTreeNode, target_values: pd.DataFrame, feature_values: pd.DataFrame, depht: int = 0):
        ...
        if gini_impurity < node_gini_value and depht < self.max_depht:
            ...
            # Todo eq 1 entfernen
            # Berechne wahrscheinlickeit zum ausgeben
            node.chance = target_values[target_values.eq(1)].count() / target_values.count()

            # Mask für neuen linken und rechten Bucket vorbereitet.
    ...
    def _evaluate_node(target_values: pd.DataFrame, feature_values: pd.DataFrame) -> Tuple[float, str, Any]:
        ...
        # Für jede Spalte wird eine gini impurity berechnet und der beste wert gewinnt.
        for current_feature_name in feature_values.columns:
            # Für numerische Spalten muss spezeiller gerechnet werden.
            

            # Berechne beste gini impurity für die Spalte
            new_gini_impurity, new_combination = DecisionTreeClassifier._calculate_gini(
                                                    current_feature_name,
                                                    target_values,
                                                    feature_values)
            
            # Nur wenn neue gini impurity keliner ist, dann soll diese verwendet werden.
    ...
    def _calculate_gini(feature_name: str, target_values: pd.DataFrame, feature_values: pd.DataFrame) -> Tuple[float, Any]:
        ...
        # Es werden zuerst die feautre Daten vorbereitet und festgestellt ob es ein numerisches Feature
        ...
        for item in np.sort(feature_data.unique()):
            ...
            # Hier werden virtuelle Linke und rechte Buckets erstllt, 
            ...
            # Es werden werte hinzugefügt wenn is in einem bucket eine 100% Wahrscheinlickeit bibt
